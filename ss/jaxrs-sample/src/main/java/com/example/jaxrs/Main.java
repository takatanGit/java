...
public class Main {
	...
	public static void main(String[] args) {
       ...
       target = client.target("http://weather.olp.yahooapis.jp")
        		.path("v1/place")
        		.queryParam("appid", "YAHOO-API-ID")
        		.queryParam("coordinates","139.732293,35.663613")
        		.queryParam("output","json")
        		//YYYYMMDDHHMI(現在日付よりに時間前まで。)
        		.queryParam("date", "201610201700")
        		.queryParam("past", o)
     ...
     // convert JSON string to Map
        ...
        try {
			...
			System.out.println(json.getFeature()[0].getProperty().getWeatherList().getWeather()[0].getDate());;
			System.out.println(json.getFeature()[0].getProperty().getWeatherAreaCode());
